#!/usr/bin/env bash
# Launch wlogout (power / lock / suspend menu), scaling margins to the screen resolution

# Base top/bottom margins per resolution tier
A_2160=450
B_2160=450
A_1600=400
B_1600=400
...
# Toggle: close wlogout if it is already open
if pgrep -x "wlogout" >/dev/null; then
    pkill -x "wlogout"
    exit 0
fi

# Detect focused monitor resolution and scaling factor
resolution=$(hyprctl -j monitors | jq -r '.[] | select(.focused==true) | .height / .scale' | awk -F'.' '{print $1}')
hypr_scale=$(hyprctl -j monitors | jq -r '.[] | select(.focused==true) | .scale')

# Scale margins to the detected resolution and launch wlogout
